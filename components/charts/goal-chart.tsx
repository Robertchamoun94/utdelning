...
export default function RaknaPage() {
  ...
  const startCapitalValue = Number(startCapital) || 0;
  const monthlySavingValue = Number(monthlySaving) || 0;
  const returnRateValue = Number(returnRate) || 0;
  const yearsValue = Math.max(0, Math.floor(Number(years) || 0));
