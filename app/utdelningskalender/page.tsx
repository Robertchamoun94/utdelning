...
export default async function UtdelningskalenderPage() {
  ...
  return (
    <>
      ...
      <section className="mx-auto w-full max-w-6xl bg-white px-4 pb-20 pt-2 text-slate-950 sm:px-6 lg:px-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {faqItems.map((item) => (
              <article
                key={item.question}
                className="rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm sm:p-5"
              >
